...
if(isset($_REQUEST["news_anx_pk"]))
   $news_anx_pk = $_REQUEST["news_anx_pk"];
...
if($l_str_acao == K_CONSULTAR || $l_str_acao == K_ALTERAR || $l_str_acao == K_EXCLUIR){
	...
	$l_obj_anexo->getAnexo($news_anx_pk);
}
...
var achou = false;
  ...
  if( (l_str_acao == '<?php echo K_INCLUIR ?>') || (l_str_acao == '<?php echo K_ALTERAR ?>') ){
     ...
     ?>  
	  ...
	  if(func_bl_isvazio(frm.news_anx_titulo,'O Titulo do anexo precisa ser preenchido!'))
	      return false;
	  

	  if(func_bl_isvazio(frm.news_anx_descricao,'A descricao do anexo precisa ser preenchido!'))
	      return false;		  
		  
      if(l_str_acao == '<?php echo K_INCLUIR ?>')
		  if(func_bl_isvazio(frm.news_anx_path,'O arquivo precisa ser anexado!'))
  	           return false;
      			   
				   
      if(!func_bl_isvazio(frm.news_anx_path)){
	         
			 if(frm.news_anx_path.value.lastIndexOf(".")<=0){
			     alert("O anexo nao possui um nome ou uma extensao!");
				 frm.news_anx_path.focus();
			     return false;
			 }
			 else{
			     ...
			     for(i in extensoes){
			         ...
			         if(  extensoes[i] ==  frm.news_anx_path.value.toUpperCase().slice(frm.news_anx_path.value.lastIndexOf(".")+1) )
			            achou = true;
			     }
	 ...
	 if((l_str_acao == '<?php echo K_INCLUIR ?>' || l_str_acao == '<?php echo K_ALTERAR ?>') && ( !func_bl_isvazio(frm.news_anx_path) )  ){

         tipo_extensao = frm.tipo_anx_fk.value.split("<?php echo K_SEPARADOR?>");

		 if(frm.news_anx_path.value.toUpperCase().slice(frm.news_anx_path.value.lastIndexOf(".")+1) != tipo_extensao[1]){
	 ...
	 if((l_str_acao == '<?php echo K_ALTERAR ?>') && ( func_bl_isvazio(frm.news_anx_path) )  ){

         tipo_extensao = frm.tipo_anx_fk.value.split("<?php echo K_SEPARADOR?>");
		 if("<?php echo $l_obj_anexo->news_anx_path;?>".toUpperCase().slice("<?php echo $l_obj_anexo->news_anx_path;?>".lastIndexOf(".")+1) != tipo_extensao[1]){
	  ...
	  if(func_bl_isvazio(frm.news_anx_credito,'O credito do anexo precisa ser preenchido!'))
	      return false;		  		  
...
<input type="hidden" name="news_anx_pk" value="<?php echo $l_obj_anexo->news_anx_pk ?>">
  ...
  <td align="center">
     <table width="530" border="0">
		...
		<tr>
			...
			<td width="630">
		      <?php if($l_str_acao == K_INCLUIR || $l_str_acao == K_ALTERAR) {?>
		         <input type="text" name="news_anx_titulo" size="40" value="<?php echo $l_obj_anexo->news_anx_titulo; ?>"> 
		      <?php }else echo $l_obj_anexo->news_anx_titulo; ?>			</td>
		</tr>
		...
		<tr>
			...
			<td>
		      <?php if($l_str_acao == K_INCLUIR || $l_str_acao == K_ALTERAR) {?>
		         <textarea name="news_anx_descricao" cols="60" rows="4"><?php echo $l_obj_anexo->news_anx_descricao; ?></textarea>
		      <?php }else echo $l_obj_anexo->news_anx_descricao; ?>		   </td>
		</tr>
		<?php if($l_str_acao != K_INCLUIR ) {?>		
	    <tr>
		   <td><b>Data cadastro:</b></td>
		   <td>
		     <?php echo $l_obj_anexo->news_anx_dt_cadastro; ?>		   </td>
		</tr>								
		<?php } ?>
	    <tr>
		   <td><b>Arquivo:</b></td>
		   <td>
		    <?php if($l_str_acao == K_INCLUIR || $l_str_acao == K_ALTERAR) {?>
		      <input type="file" name="news_anx_path" size="40" ><br>
			<?php } echo "<a id='link02' href='".K_UPLOADDIR_EXIB_ANX_NOT.$l_obj_anexo->news_anx_path."' target='_blank'>".substr($l_obj_anexo->news_anx_path,24)."</a>"; ?>		   </td>
		</tr>
	    <tr>
	    ...
	    <tr>
		   ...
		   <td>
		    <?php if($l_str_acao == K_INCLUIR || $l_str_acao == K_ALTERAR) {?>
		      <input type="text" name="news_anx_credito" size="40" value="<?php echo $l_obj_anexo->news_anx_credito; ?>">
			<?php }else echo $l_obj_anexo->news_anx_credito; ?>		   </td>
		</tr>						
		<tr><td colspan="2" align="center">
		 ...
		 <?php }?>  
           <input type='button' value="&laquo; Voltar" onClick="GoTo('ap_index_lst_anexo.php?news_not_pk=<?php echo $news_not_pk ?>&news_cat_pk=<?php echo $news_cat_pk ?>')">
        </td></tr>
	 </table>
  </td>
</tr>
